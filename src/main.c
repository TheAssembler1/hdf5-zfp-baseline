...
#define USAGE "./zfp_baseline <collective_io_enable:bool> " \
              "<chunks_per_rank:int> <scale_by_rank_enable:bool> <zfp_filter_enable:bool>\n"
...
typedef struct io_impl_funcs_t {
     ...
     * Perform any necessary global or library-wide setup before any dataset or file operations.
     * This may include initializing internal state, setting up parallel I/O contexts,
     * or checking for available features or filters.
     */
    void (*init)();
    /**
     ...
     * Set up file access properties, create or open files, define dataset dimensions and layouts,
     * and prepare any metadata needed to manage the dataset.
     *
     * Parameters:
     *  - nprocs: number of parallel processes
     *  - chunks_per_rank: number of data chunks each process will handle
     */
    void (*init_dataset)(int nprocs, int chunks_per_rank);
    /**
     * Creates the dataset within the file, using the configuration set during initialization.
     * Allocate storage, set chunking, compression, and other dataset-specific properties.
     * Must be called after dataset initialization and before any read/write operations.
     */
    void (*create_dataset)();
    /**
     * Enables compression or other filters on the dataset before creation.
     * Configure dataset creation properties to apply compression or other transforms.
     * This function is typically called after dataset initialization and before creation.
     */
     ...
     *  - collective_io: if true, use collective I/O semantics; otherwise, independent I/O
     *  - rank: identifier of the current process (e.g., MPI rank)
     *  - chunks_per_rank: total chunks assigned to each process
     *  - chunk: index of the chunk to write within the process's assigned chunks
     *
     * The implementation should select the appropriate region in the dataset and write
     * the data from the buffer according to the provided parameters.
     */
    void (*write_chunk)(float* buffer, bool collective_io, int rank, int chunks_per_rank, int chunk);
    /**
     ...
     *
     * The implementation should select the appropriate region in the dataset and read
     * the data into the provided buffer according to the parameters.
     */
    void (*read_chunk)(float* buffer, bool collective_io, int rank, int chunks_per_rank, int chunk);
    /**
     * Flushes any buffered writes to storage.
     * Ensures data consistency by committing any pending I/O operations to durable storage.
     */
    void (*flush)();
    /**
     * Closes the dataset and file handles.
     * Release all resources associated with the dataset and file, ensuring a clean shutdown.
     */
    void (*close_dataset)();
    /**
     * Reopens the dataset for further I/O after it has been closed.
     * This function should open the file and dataset (typically in read-only mode)
     * to prepare for subsequent read operations or further processing.
     */
    void (*reopen_dataset)();
} io_impl_funcs_t;
...
int main(int argc, char** argv) {
    ...
    bool zfp_compress = 0;
    io_impl_t cur_io_impl = HDF5_IMPL;
    ...
    io_impl_funcs_t io_impl_funcs[NUM_IMPL] = {
        [HDF5_IMPL] = {
            ...
            .enable_compression_on_dataset = hdf5_io_enable_compression_on_dataset,
    ...
    // init dataset
    io_impl_funcs[cur_io_impl].init_dataset(nprocs, chunks_per_rank);
    ...
    for (int c = 0; c < chunks_per_rank; c++) {
        ...
        io_impl_funcs[cur_io_impl].write_chunk(buffer, collective_io, rank, chunks_per_rank, c);
